...
def pbIsDittri?(pokemon)
  dexdata=pbOpenDexData
  pbDexDataOffset(dexdata,pokemon.species,31)
  compat10=dexdata.fgetb
  compat11=dexdata.fgetb
  dexdata.close
  return isConst?(compat10,PBEggGroups,:Dittri) ||
         isConst?(compat11,PBEggGroups,:Dittri)
end
...
def pbDayCareGetCompat
  if pbDayCareDeposited==2
    ...
    dexdata.close
    if isConst?(compat10,PBEggGroups,:Dittri) || 
       isConst?(compat11,PBEggGroups,:Dittri) ||
       isConst?(compat20,PBEggGroups,:Dittri) ||
       isConst?(compat21,PBEggGroups,:Dittri)
       ret = 3
       return ret
    end
...
def pbDayCareGenerateEgg
  ...
  father=nil
  babyspecies=0
  ditto0=pbIsDitto?(pokemon0) || pbIsDittri?(pokemon0)
  ditto1=pbIsDitto?(pokemon1) || pbIsDittri?(pokemon1)

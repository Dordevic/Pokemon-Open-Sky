...
def Kernel.pbCut
  ...
  if !pbCheckHiddenMoveBadge(BADGEFORCUT,false) || (!$DEBUG && !movefinder && $PokemonBag.pbQuantity(PBItems::EXCALIBUR)==0)
  ...
  if Kernel.pbConfirmMessage(_INTL("Would you like to cut it?"))
    if $PokemonBag.pbQuantity(PBItems::EXCALIBUR)>0
      Kernel.pbMessage(_INTL("{1} used Excalibur to cut down the tree!",$Trainer.name))
...
def useMoveCut
   Kernel.pbMessage(_INTL("{1} used Excalibur to cut the tree!",$Trainer.name))
   facingEvent = $game_player.pbFacingEvent
   if facingEvent
     pbSmashEvent(facingEvent)
   end
...
def Kernel.pbDive
  ...
  movefinder = Kernel.pbCheckMove(move)
  if !pbCheckHiddenMoveBadge(BADGEFORDIVE,false) || (!$DEBUG && !movefinder && $PokemonBag.pbQuantity(PBItems::COHULEENDRUITH)==0)
  ...
  if Kernel.pbConfirmMessage(_INTL("The sea is deep here. Would you like to use Dive?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    if $PokemonBag.pbQuantity(PBItems::COHULEENDRUITH)>0
    Kernel.pbMessage(_INTL("{1} used the Cohuleen Druith to go underwater!",$Trainer.name))
    else
    Kernel.pbMessage(_INTL("{1} used {2}!",speciesname,PBMoves.getName(move)))
    end
    pbHiddenMoveAnimation(movefinder)
...
def Kernel.pbSurfacing
  ...
  if Kernel.pbConfirmMessage(_INTL("Light is filtering down from above. Would you like to use Dive?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    if $PokemonBag.pbQuantity(PBItems::COHULEENDRUITH)>0
      Kernel.pbMessage(_INTL("{1} used the Cohuleen Druith to go to the surface!",$Trainer.name))
    else
      Kernel.pbMessage(_INTL("{1} used {2}!",speciesname,PBMoves.getName(move)))
    end
    pbHiddenMoveAnimation(movefinder)
...
def useMoveDive
   ...
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used the Cohuleen Druith!",$Trainer.name))
   end
   pbFadeOutIn(99999){
      $game_temp.player_new_map_id    = divemap
      $game_temp.player_new_x         = $game_player.x
...
def useMoveFlash
   ...
   return false if !darkness || darkness.disposed?
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used the Phoenix Feather to enlight the area!",$Trainer.name))
   end
   $PokemonGlobal.flashUsed = true
   while darkness.radius<250
     Graphics.update
...
def useMoveFly
  ...
  $PokemonTemp.flydata=ret
   ...
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used the Talaria to fly!",$Trainer.name))
   end
   pbFadeOutIn(99999){
      $game_temp.player_new_map_id    = $PokemonTemp.flydata[0]
      $game_temp.player_new_x         = $PokemonTemp.flydata[1]
...
def Kernel.pbRockSmash
  ...
  movefinder = Kernel.pbCheckMove(move)
  if !pbCheckHiddenMoveBadge(BADGEFORROCKSMASH,false) || (!$DEBUG && !movefinder && $PokemonBag.pbQuantity(PBItems::MJOLNIR)==0)
  ...
  if Kernel.pbConfirmMessage(_INTL("This rock appears to be breakable. Would you like to use Rock Smash?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    if $PokemonBag.pbQuantity(PBItems::MJOLNIR)>0
      Kernel.pbMessage(_INTL("{1} used Mjolnir to break the rocks!",$Trainer.name))
    else
      Kernel.pbMessage(_INTL("{1} used {2}!",speciesname,PBMoves.getName(move)))
    end
    pbHiddenMoveAnimation(movefinder)
...
def useMoveRockSmash
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used Mjolnir to break the rock!",$Trainer.name))
   end
   facingEvent = $game_player.pbFacingEvent
   if facingEvent
     pbSmashEvent(facingEvent)
...
def Kernel.pbStrength
  ...
  movefinder = Kernel.pbCheckMove(move)
  if !pbCheckHiddenMoveBadge(BADGEFORSTRENGTH,false) || (!$DEBUG && !movefinder && $PokemonBag.pbQuantity(PBItems::MEGINGORD)==0)
  ...
  if Kernel.pbConfirmMessage(_INTL("Would you like to use Strength?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    if $PokemonBag.pbQuantity(PBItems::MEGINGORD)>0
      Kernel.pbMessage("{1} used Megingord!",speciesname)
    else
      Kernel.pbMessage(_INTL("{1} used {2}!",speciesname,PBMoves.getName(move)))
    end
    pbHiddenMoveAnimation(movefinder)
...
def useMoveStrength
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used Megingord!",$Trainer.name))
   end
   Kernel.pbMessage(_INTL("{1}'s Strength made it possible to move boulders around!",$Trainer.name))
   $PokemonMap.strengthUsed = true
   return true
end
...
def Kernel.pbSurf
  ...
  movefinder = Kernel.pbCheckMove(move)
  if !pbCheckHiddenMoveBadge(BADGEFORSURF,false) || (!$DEBUG && !movefinder && $PokemonBag.pbQuantity(PBItems::POSEIDONSTRIDENT)==0)
    Kernel.pbMessage(_INTL("The water is a deep blue. Maybe a Pokémon can help you travel through it"))
    return false
  end
  if Kernel.pbConfirmMessage(_INTL("The water is a deep blue...\nWould you like to surf on it?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    if $PokemonBag.pbQuantity(PBItems::POSEIDONSTRIDENT)>0
      Kernel.pbMessage(_INTL("{1} used Poseidon's Trident to cross the water!",$Trainer.name))
    else
      Kernel.pbMessage(_INTL("{1} used {2}!",speciesname,PBMoves.getName(move)))
    end
    Kernel.pbCancelVehicles
...
def Kernel.pbWaterfall
  ...
  movefinder = Kernel.pbCheckMove(move)
  if !pbCheckHiddenMoveBadge(BADGEFORWATERFALL,false) || (!$DEBUG && !movefinder && $PokemonBag.pbQuantity(PBItems::MOSESSTAFF)==0)
  ...
  if Kernel.pbConfirmMessage(_INTL("It's a large waterfall. Would you like to use Waterfall?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    if $PokemonBag.pbQuantity(PBItems::MOSESSTAFF)>0
      Kernel.pbMessage(_INTL("{1} used Moses' Staff to climb the waterfall!",$Trainer.name))
    else
      Kernel.pbMessage(_INTL("{1} used {2}!",speciesname,PBMoves.getName(move)))
    end
    pbHiddenMoveAnimation(movefinder)

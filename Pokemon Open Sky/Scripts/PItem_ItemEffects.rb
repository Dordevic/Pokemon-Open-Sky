...
ItemHandlers::UseFromBag.add(:EXCALIBUR,proc{|item|
   next canUseMoveCut? ? 2 : 0
})

ItemHandlers::UseFromBag.add(:COHULEENDRUITH,proc{|item|
   next canUseMoveDive? ? 2 : 0
})

ItemHandlers::UseFromBag.add(:PHOENIXFEATHER,proc{|item|
   next canUseMoveFlash? ? 2 : 0
})

ItemHandlers::UseFromBag.add(:TALARIA,proc{|item|
   next canUseMoveFly? ? 2 : 0
})

ItemHandlers::UseFromBag.add(:MJOLNIR,proc{|item|
   next canUseMoveRockSmash? ? 2 : 0
})

ItemHandlers::UseFromBag.add(:MEGINGORD,proc{|item|
   next canUseMoveStrength? ? 2 : 0
})

ItemHandlers::UseFromBag.add(:POSEIDONSTRIDENT,proc{|item|
   next canUseMoveSurf? ? 2 : 0
})

ItemHandlers::UseFromBag.add(:MOSESSTAFF,proc{|item|
...
ItemHandlers::UseInField.add(:YELLOWHAIRDYE,proc{|item|
   $game_variables[99]= 1
   $Trainer.outfit = $game_variables[99]*10+$game_variables[100]
   Kernel.pbMessage(_INTL("The Exp Share was turned on."))
   next 1
})

ItemHandlers::UseInField.add(:EXCALIBUR,proc{|item|
   useMoveCut if canUseMoveCut?
})

ItemHandlers::UseInField.add(:COHULEENDRUITH,proc{|item|
   useMoveDive if canUseMoveDive?
})

ItemHandlers::UseInField.add(:PHOENIXFEATHER,proc{|item|
   useMoveFlash if canUseMoveFlash?
})

ItemHandlers::UseInField.add(:TALARIA,proc{|item|
   useMoveFly if canUseMoveFly?
})

ItemHandlers::UseInField.add(:MJOLNIR,proc{|item|
   useMoveRockSmash if canUseMoveRockSmash?
})

ItemHandlers::UseInField.add(:MEGINGORD,proc{|item|
   useMoveStrength if canUseMoveStrength?
})

ItemHandlers::UseInField.add(:POSEIDONSTRIDENT,proc{|item|
   useMoveSurf if canUseMoveSurf?
})

ItemHandlers::UseInField.add(:MOSESSTAFF,proc{|item|
   useMoveWaterfall if canUseMoveWaterfall?
})

ItemHandlers::UseInField.add(:QUESTLOG,proc{|item|
   pbViewQuests
   next 1
})
...
ItemHandlers::UseOnPokemon.add(:STAMINAGENE,proc{|item,pokemon,scene|
   if pbRaiseIndividualValues(pokemon,PBStats::HP,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s HP potential increased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:COMBATGENE,proc{|item,pokemon,scene|
   if pbRaiseIndividualValues(pokemon,PBStats::ATTACK,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Attack potential increased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:PROTECTIVEGENE,proc{|item,pokemon,scene|
   if pbRaiseIndividualValues(pokemon,PBStats::DEFENSE,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Defense potential increased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:FOCUSGENE,proc{|item,pokemon,scene|
   if pbRaiseIndividualValues(pokemon,PBStats::SPATK,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Sp.Atk potential increased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:BARRIERGENE,proc{|item,pokemon,scene|
   if pbRaiseIndividualValues(pokemon,PBStats::SPDEF,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Sp.Def potential increased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:NIMBLEGENE,proc{|item,pokemon,scene|
   if pbRaiseIndividualValues(pokemon,PBStats::SPEED,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Speed potential increased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:LETHARGYGENE,proc{|item,pokemon,scene|
   if pbDecreaseIndividualValues(pokemon,PBStats::HP,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s HP potential decreased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:WEAKNESSGENE,proc{|item,pokemon,scene|
   if pbDecreaseIndividualValues(pokemon,PBStats::ATTACK,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Attack potential decreased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:CARELESSGENE,proc{|item,pokemon,scene|
   if pbDecreaseIndividualValues(pokemon,PBStats::DEFENSE,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Defense potential decreased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:COWARDICEGENE,proc{|item,pokemon,scene|
   if pbDecreaseIndividualValues(pokemon,PBStats::SPATK,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Sp.Atk potential decreased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:NEGLIGENTGENE,proc{|item,pokemon,scene|
   if pbDecreaseIndividualValues(pokemon,PBStats::SPDEF,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Sp.Def potential decreased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

ItemHandlers::UseOnPokemon.add(:CLUMSYGENE,proc{|item,pokemon,scene|
   if pbDecreaseIndividualValues(pokemon,PBStats::SPEED,1)==0
     scene.pbDisplay(_INTL("It won't have any effect."))
     next false
   else
     scene.pbDisplay(_INTL("{1}'s Speed potential decreased.",pokemon.name))
     pokemon.changeHappiness("vitamin")
     next true
   end
})

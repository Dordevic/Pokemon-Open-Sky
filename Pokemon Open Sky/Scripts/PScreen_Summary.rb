...
class PokemonSummary_Scene
  ...
  def pbStartScene(party,partyindex,battle)
  ...
  def drawPageThree
    ...
    # Write various bits of text
    textpos = [
       [_INTL("HP"),248,76,0,base,shadow],
       [sprintf("%d/%d(%d/%d)",@pokemon.hp,@pokemon.totalhp,@pokemon.ev[0],@pokemon.iv[0]),482,76,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Attack"),248,120,0,base,statshadows[0]],
       [sprintf("%d(%d/%d)",@pokemon.attack,@pokemon.ev[1],@pokemon.iv[1]),482,120,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Defense"),248,152,0,base,statshadows[1]],
       [sprintf("%d(%d/%d)",@pokemon.defense,@pokemon.ev[2],@pokemon.iv[2]),482,152,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Sp. Atk"),248,184,0,base,statshadows[3]],
       [sprintf("%d(%d/%d)",@pokemon.spatk,@pokemon.ev[3],@pokemon.iv[3]),482,184,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Sp. Def"),248,216,0,base,statshadows[4]],
       [sprintf("%d(%d/%d)",@pokemon.spdef,@pokemon.ev[4],@pokemon.iv[4]),482,216,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Speed"),248,248,0,base,statshadows[2]],
       [sprintf("%d(%d/%d)",@pokemon.speed,@pokemon.ev[5],@pokemon.iv[5]),482,248,1,Color.new(64,64,64),Color.new(176,176,176)],
    ...
    if @battle != nil
      for i in 0...@battle.battlers.length
        if @battle.battlers[i].pokemon!= nil && @battle.battlers[i].pokemon.personalID == @pokemon.personalID
          if @battle.battlers[i].stages[PBStats::ATTACK]<0
            textpos[3] = sprintf("%d %d",@pokemon.attack,@battle.battlers[i].stages[PBStats::ATTACK]),482,120,1,Color.new(64,64,64),Color.new(176,176,176)            
          else
            textpos[3] = sprintf("%d +%d",@pokemon.attack,@battle.battlers[i].stages[PBStats::ATTACK]),482,120,1,Color.new(64,64,64),Color.new(176,176,176)
          end
          if @battle.battlers[i].stages[PBStats::DEFENSE]<0
            textpos[5] = sprintf("%d %d",@pokemon.defense,@battle.battlers[i].stages[PBStats::DEFENSE]),482,152,1,Color.new(64,64,64),Color.new(176,176,176)
          else
            textpos[5] = sprintf("%d +%d",@pokemon.defense,@battle.battlers[i].stages[PBStats::DEFENSE]),482,152,1,Color.new(64,64,64),Color.new(176,176,176)
          end
          if @battle.battlers[i].stages[PBStats::SPATK]<0
            textpos[7] = sprintf("%d %d",@pokemon.spatk,@battle.battlers[i].stages[PBStats::SPATK]),482,184,1,Color.new(64,64,64),Color.new(176,176,176)
          else
            textpos[7] = sprintf("%d +%d",@pokemon.spatk,@battle.battlers[i].stages[PBStats::SPATK]),482,184,1,Color.new(64,64,64),Color.new(176,176,176)
          end
          if @battle.battlers[i].stages[PBStats::SPDEF]<0
            textpos[9] = sprintf("%d %d",@pokemon.spdef,@battle.battlers[i].stages[PBStats::SPDEF]),482,216,1,Color.new(64,64,64),Color.new(176,176,176)
          else
            textpos[9] = sprintf("%d +%d",@pokemon.spdef,@battle.battlers[i].stages[PBStats::SPDEF]),482,216,1,Color.new(64,64,64),Color.new(176,176,176)
          end
          if @battle.battlers[i].stages[PBStats::SPEED]<0
            textpos[11] = sprintf("%d %d",@pokemon.speed,@battle.battlers[i].stages[PBStats::SPEED]),482,248,1,Color.new(64,64,64),Color.new(176,176,176)
          else
            textpos[11] = sprintf("%d +%d",@pokemon.speed,@battle.battlers[i].stages[PBStats::SPEED]),482,248,1,Color.new(64,64,64),Color.new(176,176,176)
          end
        end
...
class PokemonSummaryScreen
  ...
  def pbStartScreen(party,partyindex,battle)

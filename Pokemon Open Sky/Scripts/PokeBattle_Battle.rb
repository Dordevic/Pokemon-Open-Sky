...
#==================
    ...
    elsif @weather==PBWeather::THUNDERSTORM
      pbCommonAnimation("Thunderstorm",nil,nil)
      pbDisplay(_INTL("It is raining cats and dogs."))
    ...
    elsif @weather==PBWeather::PLAGUE
      pbCommonAnimation("Plague",nil,nil)
      pbDisplay(_INTL("A plague wind is raging."))
    elsif @weather==PBWeather::ECLIPSE
      pbCommonAnimation("Eclipse",nil,nil)
      pbDisplay(_INTL("The field is covered in shadows."))
    elsif @weather==PBWeather::ANCESTRALAURORA
      pbCommonAnimation("AncestralAurora",nil,nil)
      pbDisplay(_INTL("An ancestral aurora is covering the field."))
    elsif @weather==PBWeather::MAGICWIND
      pbCommonAnimation("MagicWind",nil,nil)
      pbDisplay(_INTL("A magic wind is covering the field."))
    elsif @weather==PBWeather::FERALBREEZE
      pbCommonAnimation("FeralBreeze",nil,nil)
      pbDisplay(_INTL("A soft but unnerving breeze is blowing."))
    elsif @weather==PBWeather::CROWD
      pbCommonAnimation("Crowd",nil,nil)
      pbDisplay(_INTL("An enthusiastic crowd is cheering."))
    elsif @weather==PBWeather::TORNADO
      pbCommonAnimation("Tornado",nil,nil)
      pbDisplay(_INTL("A small tornado rages on the field."))
    elsif @weather==PBWeather::SCARYFOG
      pbCommonAnimation("ScaryFog",nil,nil)
      pbDisplay(_INTL("A creepy intense fog surrounds the field."))
    elsif @weather==PBWeather::POLLENSEASON
      pbCommonAnimation("PollenSeason",nil,nil)
      pbDisplay(_INTL("A lot of pollen particles surrounds the field."))
    elsif @weather==PBWeather::TREMORS
      pbCommonAnimation("Tremors",nil,nil)
      pbDisplay(_INTL("The ground is shaking."))
    elsif @weather==PBWeather::STRONGBLIZZARD
      pbCommonAnimation("StrongBlizzard",nil,nil)
      pbDisplay(_INTL("A strong blizzard is roaring."))
    elsif @weather==PBWeather::ACIDRAIN
      pbCommonAnimation("AcidRain",nil,nil)
      pbDisplay(_INTL("Poisonus and flamable rain is falling."))
    elsif @weather==PBWeather::STARRYNIGHT
      pbCommonAnimation("StarryNight",nil,nil)
      pbDisplay(_INTL("A bright and wierd nightfall covers the field."))
    elsif @weather==PBWeather::METEORSHOWER
      pbCommonAnimation("MeteorShower",nil,nil)
      pbDisplay(_INTL("Meteors are bombarding the field."))
    elsif @weather==PBWeather::MAGNETICSTORM
      pbCommonAnimation("MagneticStorm",nil,nil)
      pbDisplay(_INTL("Magnetism covers the field."))
    end
...
#        pbDisplay(_INTL("Rain continues to fall."))
    ...
    when PBWeather::THUNDERSTORM
      ...
      if @weatherduration==0
        pbDisplay(_INTL("The rain stopped."))
        @weather=0
        PBDebug.log("[End of effect] Thunderstorm weather ended")
      else
        pbCommonAnimation("Thunderstorm",nil,nil)
#        pbDisplay(_INTL("Thunderstorm is ragging."))
      end
...
#        pbDisplay(_INTL("Hail continues to fall."))
        if pbWeather==PBWeather::HAIL
          ...
          for i in priority
            next if i.fainted?
            if !i.pbHasType?(:ICE) && !i.pbHasType?(:FAIRY) && !i.pbHasType?(:FIRE) &&
      ...
      when PBWeather::PLAGUE
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The plague wind subsided."))
        @weather=0
        PBDebug.log("[End of effect] Plague weather ended")
      else
        pbCommonAnimation("Plague",nil,nil)
#        pbDisplay(_INTL("The sandstorm rages."))
        if pbWeather==PBWeather::PLAGUE
          PBDebug.log("[Lingering effect triggered] Plague weather damage")
          for i in priority
            next if i.fainted?
            if !i.pbHasType?(:BUG) && !i.pbHasType?(:POISON) && !i.pbHasType?(:FIRE) &&
               !i.hasWorkingAbility(:MAGICGUARD) &&
               !i.hasWorkingAbility(:OVERCOAT) &&
               !i.hasWorkingItem(:SAFETYGOGGLES) &&
               ![0xCA,0xCB].include?(PBMoveData.new(i.effects[PBEffects::TwoTurnAttack]).function) # Dig, Dive
              @scene.pbDamageAnimation(i,0)
              if i.pbHasType?(:FIGHTING) || i.pbHasType?(:FERAL)
                  i.pbReduceHP((i.totalhp/8).floor)
              else
                  i.pbReduceHP((i.totalhp/16).floor)
              end
              pbDisplay(_INTL("{1} is buffeted by the plague!",i.pbThis))
              if i.fainted?
                return if !i.pbFaint
              end
            end
    ...
    when PBWeather::ECLIPSE
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("Light has returned to the field."))
        @weather=0
        PBDebug.log("[End of effect] Eclipse weather ended")
      else
        pbCommonAnimation("Eclipse",nil,nil)
#        pbDisplay(_INTL("Darkness is covering the field."))
      end
    when PBWeather::ANCESTRALAURORA
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The ancestral aurora fadded."))
        @weather=0
        PBDebug.log("[End of effect] Ancestral aurora weather ended")
      else
        pbCommonAnimation("AncestralAurora",nil,nil)
#        pbDisplay(_INTL("The draconic aurora is looming over the field."))
      end
    when PBWeather::MAGICWIND
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The magic wind fadded."))
        @weather=0
        PBDebug.log("[End of effect] magic wind weather ended")
      else
        pbCommonAnimation("MagicWind",nil,nil)
#        pbDisplay(_INTL("The magic wind is blowing through the field."))
      end
    when PBWeather::FERALBREEZE
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The unnerving breeze fadded."))
        @weather=0
        PBDebug.log("[End of effect] feral breeze weather ended")
      else
        pbCommonAnimation("FeralBreeze",nil,nil)
#        pbDisplay(_INTL("The feral breeze is blowing through the field."))
      end
    when PBWeather::CROWD
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The crowd got bored and dispersed."))
        @weather=0
        PBDebug.log("[End of effect] crowd weather ended")
      else
        pbCommonAnimation("Crowd",nil,nil)
#        pbDisplay(_INTL("The crowd is cheering."))
      end
    when PBWeather::TORNADO
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The tornado fadded."))
        @weather=0
        PBDebug.log("[End of effect] tornado weather ended")
      else
        pbCommonAnimation("Tornado",nil,nil)
#        pbDisplay(_INTL("The tornado keeps going strong."))
        if pbWeather==PBWeather::TORNADO
          PBDebug.log("[Lingering effect triggered] tornado weather damage")
          for i in priority
            next if i.fainted?
            if !i.pbHasType?(:FLYING) && !i.pbHasType?(:DRAGON) && !i.pbHasType?(:STEEL) &&
               !i.hasWorkingAbility(:MAGICGUARD) &&
               !i.hasWorkingAbility(:OVERCOAT) &&
               !i.hasWorkingItem(:SAFETYGOGGLES) &&
               ![0xCA,0xCB].include?(PBMoveData.new(i.effects[PBEffects::TwoTurnAttack]).function) # Dig, Dive
              @scene.pbDamageAnimation(i,0)
              if i.pbHasType?(:GROUND) || i.pbHasType?(:ICE)
                  i.pbReduceHP((i.totalhp/8).floor)
              else
                  i.pbReduceHP((i.totalhp/16).floor)
              end
              pbDisplay(_INTL("{1} is buffeted by the debris!",i.pbThis))
              if i.fainted?
                return if !i.pbFaint
              end
            end
    ...
    when PBWeather::SCARYFOG
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The scary fog fadded."))
        @weather=0
        PBDebug.log("[End of effect] scary fog weather ended")
      else
        pbCommonAnimation("ScaryFog",nil,nil)
#        pbDisplay(_INTL("The scary fog covers the field."))
      end
    when PBWeather::POLLENSEASON
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The pollen disappeared."))
        @weather=0
        PBDebug.log("[End of effect] pollen season weather ended")
      else
        pbCommonAnimation("PollenSeason",nil,nil)
#        pbDisplay(_INTL("The pollen is floating in the air."))
      end
    when PBWeather::TREMORS
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The tremors fadded."))
        @weather=0
        PBDebug.log("[End of effect] tremors weather ended")
      else
        pbCommonAnimation("Tremors",nil,nil)
#        pbDisplay(_INTL("The pollen is floating in the air."))
        if pbWeather==PBWeather::TREMORS
          PBDebug.log("[Lingering effect triggered] tremors weather damage")
          for i in priority
            next if i.fainted?
            if !i.pbHasType?(:GROUND) && !i.pbHasType?(:ROCK) && !i.pbHasType?(:FLYING) &&
               !i.hasWorkingAbility(:MAGICGUARD) &&
               !i.hasWorkingAbility(:OVERCOAT) &&
               !i.hasWorkingItem(:SAFETYGOGGLES) &&
               ![0xCB].include?(PBMoveData.new(i.effects[PBEffects::TwoTurnAttack]).function) # Dive
              @scene.pbDamageAnimation(i,0)
              if i.pbHasType?(:POISON) || i.pbHasType?(:STEEL) || [0xCA].include?(PBMoveData.new(i.effects[PBEffects::TwoTurnAttack]).function)
                  i.pbReduceHP((i.totalhp/8).floor)
              else
                  i.pbReduceHP((i.totalhp/16).floor)
              end
              pbDisplay(_INTL("{1} is buffeted by the tremors!",i.pbThis))
              if i.fainted?
                return if !i.pbFaint
              end
            end
    ...
    when PBWeather::STRONGBLIZZARD
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The blizzard fadded."))
        @weather=0
        PBDebug.log("[End of effect] strong blizzard weather ended")
      else
        pbCommonAnimation("StrongBlizzard",nil,nil)
#        pbDisplay(_INTL("The snow is falling heavily."))
        if pbWeather==PBWeather::STRONGBLIZZARD
          PBDebug.log("[Lingering effect triggered] Plague weather damage")
          for i in priority
            next if i.fainted?
            if !i.pbHasType?(:ICE) && !i.pbHasType?(:FAIRY) &&
               !i.hasWorkingAbility(:MAGICGUARD) &&
               !i.hasWorkingAbility(:OVERCOAT) &&
               !i.hasWorkingItem(:SAFETYGOGGLES) &&
               ![0xCA,0xCB].include?(PBMoveData.new(i.effects[PBEffects::TwoTurnAttack]).function) # Dig, Dive
              @scene.pbDamageAnimation(i,0)
              if i.pbHasType?(:GRASS) || i.pbHasType?(:GROUND)
                  i.pbReduceHP((i.totalhp/8).floor)
              else
                  i.pbReduceHP((i.totalhp/16).floor)
              end
              pbDisplay(_INTL("{1} is buffeted by the snow!",i.pbThis))
              if i.fainted?
                return if !i.pbFaint
              end
            end
    ...
    when PBWeather::ACIDRAIN
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The acid rain fadded."))
        @weather=0
        PBDebug.log("[End of effect] pollen season weather ended")
      else
        pbCommonAnimation("AcidRain",nil,nil)
#        pbDisplay(_INTL("The pollen is floating in the air."))
        if pbWeather==PBWeather::ACIDRAIN
          PBDebug.log("[Lingering effect triggered] Plague weather damage")
          for i in priority
            next if i.fainted?
            if !i.pbHasType?(:POISON) && !i.pbHasType?(:FIRE) &&
               !i.hasWorkingAbility(:MAGICGUARD) &&
               !i.hasWorkingAbility(:OVERCOAT) &&
               !i.hasWorkingItem(:SAFETYGOGGLES) &&
               ![0xCA,0xCB].include?(PBMoveData.new(i.effects[PBEffects::TwoTurnAttack]).function) # Dig, Dive
              @scene.pbDamageAnimation(i,0)
              if i.pbHasType?(:GRASS) || i.pbHasType?(:ICE)
                  i.pbReduceHP((i.totalhp/8).floor)
              else
                  i.pbReduceHP((i.totalhp/16).floor)
              end
              pbDisplay(_INTL("{1} is corroded by the acid rain!",i.pbThis))
              if i.fainted?
                return if !i.pbFaint
              end
            end
    ...
    when PBWeather::STARRYNIGHT
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The night fadded."))
        @weather=0
        PBDebug.log("[End of effect] starry night weather ended")
      else
        pbCommonAnimation("StarryNight",nil,nil)
#        pbDisplay(_INTL("The stars are shining."))
      end
    when PBWeather::METEORSHOWER
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The meteor shower stopped."))
        @weather=0
        PBDebug.log("[End of effect] meteor shower weather ended")
      else
        pbCommonAnimation("MeteorShower",nil,nil)
#        pbDisplay(_INTL("Meteores are bombarding the field."))
        if pbWeather==PBWeather::METEORSHOWER
          PBDebug.log("[Lingering effect triggered] Plague weather damage")
          for i in priority
            next if i.fainted?
            if !i.pbHasType?(:STEEL) && !i.pbHasType?(:ROCK) &&
               !i.hasWorkingAbility(:MAGICGUARD) &&
               !i.hasWorkingAbility(:OVERCOAT) &&
               !i.hasWorkingItem(:SAFETYGOGGLES) &&
               ![0xCA,0xCB].include?(PBMoveData.new(i.effects[PBEffects::TwoTurnAttack]).function) # Dig, Dive
              @scene.pbDamageAnimation(i,0)
              if i.pbHasType?(:BUG) || i.pbHasType?(:FLYING)
                  i.pbReduceHP((i.totalhp/8).floor)
              else
                  i.pbReduceHP((i.totalhp/16).floor)
              end
              pbDisplay(_INTL("{1} is hit by the meteors!",i.pbThis))
              if i.fainted?
                return if !i.pbFaint
              end
            end
    ...
    when PBWeather::MAGNETICSTORM
      @weatherduration=@weatherduration-1 if @weatherduration>0
      if @weatherduration==0
        pbDisplay(_INTL("The magnetism fadded."))
        @weather=0
        PBDebug.log("[End of effect] starry night weather ended")
      else
        pbCommonAnimation("MagneticStorm",nil,nil)
#        pbDisplay(_INTL("The magnetism is going strong."))
      end
    when PBWeather::HEAVYRAIN
      hasabil=false
      for i in 0...4
...
#        pbDisplay(_INTL("The shadow sky continues."));
    ...
    for i in priority
      ...
      if (i.hasWorkingAbility(:SHEDSKIN) && pbRandom(10)<3) ||
        ...
        if i.status>0
          ...
          when PBStatuses::WET
            pbDisplay(_INTL("{1}'s {2} dry it!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::RASH
            pbDisplay(_INTL("{1}'s {2} cured its rash!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::BRUISED
            pbDisplay(_INTL("{1}'s {2} cured its bruises!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::DIRTY
            pbDisplay(_INTL("{1}'s {2} cleaned the dirt!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::UNBLANCED
            pbDisplay(_INTL("{1}'s {2} restored its balance!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::MINDCONTROL
            pbDisplay(_INTL("{1}'s {2} restored its mind!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::STUNG
            pbDisplay(_INTL("{1}'s {2} removed the stings!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::PETRIFY
            pbDisplay(_INTL("{1}'s {2} restored its mobility!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::FEAR
            pbDisplay(_INTL("{1}'s {2} drove away its fear!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::AWAKENED
            pbDisplay(_INTL("{1}'s {2} locked its hidden power!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::BLINDED
            pbDisplay(_INTL("{1}'s {2} cleared its eyes!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::SHRAPNEL
            pbDisplay(_INTL("{1}'s {2} removed the shrapnel!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::ENCHANTED
            pbDisplay(_INTL("{1}'s {2} removed the enchantment!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::BLIEEDING
            pbDisplay(_INTL("{1}'s {2} cured its wounds!",i.pbThis,PBAbilities.getName(i.ability)))
          end
        end
      end
      # Healer
      if i.hasWorkingAbility(:HEALER) && pbRandom(10)<3
        ...
        if partner && partner.status>0
          ...
          when PBStatuses::WET
            pbDisplay(_INTL("{1}'s {2} dried its partner's!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::RASH
            pbDisplay(_INTL("{1}'s {2} cured its partner's rash problem!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::BRUISED
            pbDisplay(_INTL("{1}'s {2} healed its partner's bruises!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::DIRTY
            pbDisplay(_INTL("{1}'s {2} cleaned its partner!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::UNBALANCED
            pbDisplay(_INTL("{1}'s {2} cured its partner's unbalance problem!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::MINDCONTROL
            pbDisplay(_INTL("{1}'s {2} cured its partner's mind problem!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::STUNG
            pbDisplay(_INTL("{1}'s {2} cured its partner's sting problem!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::PETRIFY
            pbDisplay(_INTL("{1}'s {2} healed its partner's petrification!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::FEAR
            pbDisplay(_INTL("{1}'s {2} cured its partner's fear!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::AWAKENED
            pbDisplay(_INTL("{1}'s {2} stopped its partner awakening!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::BLINDED
            pbDisplay(_INTL("{1}'s {2} cured its partner's sight problem!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::SHRAPNEL
            pbDisplay(_INTL("{1}'s {2} cured its partner's shrapnel problem!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::ENCHANTED
            pbDisplay(_INTL("{1}'s {2} healed its partner's enchantment!",i.pbThis,PBAbilities.getName(i.ability)))
          when PBStatuses::BLEEDING
            pbDisplay(_INTL("{1}'s {2} cured its partner's bleeding!",i.pbThis,PBAbilities.getName(i.ability)))
          end
        end
      end
    end
    ...
    for i in priority
      ...
      # Rash
      if i.status==PBStatuses::RASH
        if !i.hasWorkingAbility(:MAGICGUARD)
          PBDebug.log("[Status damage] #{i.pbThis} took damage from rash")
          if i.hasWorkingAbility(:HEATPROOF) #TODO: change ability
            PBDebug.log("[Ability triggered] #{i.pbThis}'s Heatproof")
            i.pbReduceHP((i.totalhp/16).floor)
          else
            i.pbReduceHP((i.totalhp/8).floor)
      ...
      # Bleeding
      if i.status==PBStatuses::BLEEDING
        if !i.hasWorkingAbility(:MAGICGUARD)
          PBDebug.log("[Status damage] #{i.pbThis} took damage from bleeding")
          if i.hasWorkingAbility(:HEATPROOF) #TODO: change ability
            PBDebug.log("[Ability triggered] #{i.pbThis}'s Heatproof")
            i.pbReduceHP((i.totalhp/16).floor)
          else
      ...
      # Dirty
      if i.status==PBStatuses::DIRTY
        if !i.hasWorkingAbility(:MAGICGUARD)
          PBDebug.log("[Status damage] #{i.pbThis} took damage from dirt")
          if i.hasWorkingAbility(:HEATPROOF) #TODO: change ability
            PBDebug.log("[Ability triggered] #{i.pbThis}'s Heatproof")
      ...
      # Shrapnel
      if i.status==PBStatuses::SHRAPNEL
        if !i.hasWorkingAbility(:MAGICGUARD)
          PBDebug.log("[Status damage] #{i.pbThis} took damage from the shrapnel")
          if i.hasWorkingAbility(:HEATPROOF) #TODO: change ability
            PBDebug.log("[Ability triggered] #{i.pbThis}'s Heatproof")
            i.pbReduceHP((i.totalhp/12).floor)
          else
      ...
      # Enchanted
      if i.status==PBStatuses::ENCHANTED
        PBDebug.log("[Status damage] #{i.pbThis} recovered health from enchantment")
        if i.hasWorkingAbility(:HEALEAR) #TODO: change ability
          PBDebug.log("[Ability triggered] #{i.pbThis}'s Heatproof")
          i.pbRecoverHP((i.totalhp/6).floor)
        else
      ...
      # Awakened
      if i.status==PBStatuses::AWAKENED
        i.pbContinueStatus
        if !i.hasWorkingAbility(:MAGICGUARD)
          PBDebug.log("[Status damage] #{i.pbThis} took damage from awakeninig")
          i.pbReduceHP(((i.totalhp*i.effects[PBEffects::Toxic])/16).floor)
            pbDisplay(_INTL("{1} is hur by his awakening!",i.pbThis))
        end
        if i.pbCanIncreaseStatStage?(PBStats::ATTACK,1,i,false,self)
          i.pbIncreaseStat(PBStats::ATTACK,1,i,false,self)
        end
        if i.pbCanIncreaseStatStage?(PBStats::DEFENSE,1,i,false,self)
          i.pbIncreaseStat(PBStats::DEFENSE,1,i,false,self)
        end
        if i.pbCanIncreaseStatStage?(PBStats::SPATK,1,i,false,self)
          i.pbIncreaseStat(PBStats::SPATK,1,i,false,self)
        end
        if i.pbCanIncreaseStatStage?(PBStats::SPDEF,1,i,false,self)
          i.pbIncreaseStat(PBStats::SPDEF,1,i,false,self)
        end
        if i.pbCanIncreaseStatStage?(PBStats::SPEED,1,i,false,self)
          i.pbIncreaseStat(PBStats::SPEED,1,i,false,self)
        end
        if i.statusCount>0
          i.effects[PBEffects::Toxic]+=1
          pbDisplay(_INTL("Toxic went up to {1}!",i.effects[PBEffects::Toxic]))
          i.effects[PBEffects::Toxic]=[15,i.effects[PBEffects::Toxic]].min
        end
      end
      # Nightmare
    ...
    #TODO: Healing Wish/Lunar Dance - should go here
    #TODO: Spikes/Toxic Spikes/Stealth Rock - should go here (in order of their 1st use)
    for i in 0...4

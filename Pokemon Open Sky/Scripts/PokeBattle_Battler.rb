...
################################################################################
  ...
  def pbSpeed
    ...
    when PBWeather::SUNNYDAY, PBWeather::HARSHSUN
      speedmult=speedmult*1.2 if self.pbHasType?(:DRAGON)
      speedmult=speedmult*2 if self.hasWorkingAbility(:CHLOROPHYLL)
    when PBWeather::SANDSTORM
      speedmult=speedmult*2 if self.hasWorkingAbility(:SANDRUSH)
    when PBWeather::HAIL, PBWeather::STRONGBLIZZARD
      speedmult=speedmult*0.8 if self.pbHasType?(:DRAGON)
    when PBWeather::MAGNETICSTORM
      speedmult=speedmult*1.2 if self.pbHasType?(:STEEL)
    when PBWeather::METEORSHOWER
      speedmult=speedmult*0.8 if self.pbHasType?(:FLYING)
    when PBWeather::TREMORS
      speedmult=speedmult*0.8 if !self.pbHasType?(:GROUND)
    when PBWeather::CROWD
      speedmult=speedmult*1.2 if self.pbHasType?(:FIGHTING)
      speedmult=speedmult*0.8 if self.pbHasType?(:DARK)      
    end
...
################################################################################
  ...
  def pbTryUseMove(choice,thismove,turneffects)
    ...
    if !turneffects[PBEffects::SkipAccuracyCheck]
      ...
      if self.status==PBStatuses::MINDCONTROL && thismove.pbIsSpecial?(thismove.type)
        if @battle.pbRandom(2)==0
          pbContinueStatus
          PBDebug.log("[Status] #{pbThis} lost its balance and couldn't move")
          return false
        end
      end
      if self.status==PBStatuses::UNBALANCED && thismove.pbIsPhysical?(thismove.type)
        if @battle.pbRandom(2)==0
          pbContinueStatus
          PBDebug.log("[Status] #{pbThis} lost its balance and couldn't move")
          return false
      ...
      if self.status==PBStatuses::FEAR && thismove.pbIsSpecial?(thismove.type)
        pbContinueStatus
        PBDebug.log("[Status] #{pbThis} is scarred and couldn't move")
        return false
      end
      if self.status==PBStatuses::PETRIFY && thismove.pbIsPhysical?(thismove.type)
        pbContinueStatus
        PBDebug.log("[Status] #{pbThis} is petrified and couldn't move")
        return false
      end
    end
    if !turneffects[PBEffects::SkipAccuracyCheck]
  ...
  def pbProcessMoveAgainstTarget(thismove,user,target,numhits,turneffects,nocheck=false,alltargets=nil,showanimation=true)
    ...
    for i in 0...numhits
      ...
      if !nocheck &&
        ...
        elsif user.status==PBStatuses::WET && thismove.pbIsSpecial?(thismove.type)
          @battle.pbCommonAnimation("Wet",self,nil)
          @battle.pbDisplay(_INTL("{1} slipped on the water and hurt itself!",pbThis))
          damage=(user.totalhp/6).floor
          if damage>0
            @battle.scene.pbDamageAnimation(user,0)
            user.pbReduceHP(damage)
          end
          user.pbFaint if user.fainted?
        elsif user.status==PBStatuses::BLINDED && thismove.pbIsPhysical?(thismove.type)
          @battle.pbCommonAnimation("Blinded",self,nil)
          @battle.pbDisplay(_INTL("{1} missed the opponent and crashed!",pbThis))
          damage=(user.totalhp/6).floor
          if damage>0
             @battle.scene.pbDamageAnimation(user,0)
             user.pbReduceHP(damage)
    ...
    #Stung and Bruise damage
    if user.status==PBStatuses::STUNG && thismove.pbIsSpecial?(thismove.type)
      @battle.pbCommonAnimation("Stung",self,nil)
      @battle.pbDisplay(_INTL("{1}'s stings hurt it",pbThis))
      damage=(user.totalhp/6).floor
      if damage>0
        @battle.scene.pbDamageAnimation(user,0)
        user.pbReduceHP(damage)
      ...
      user.pbFaint if user.fainted?
    elsif user.status==PBStatuses::BRUISED && thismove.pbIsPhysical?(thismove.type)
      @battle.pbCommonAnimation("Bruised",self,nil)
      @battle.pbDisplay(_INTL("{1}'s bruises hurt it!",pbThis))
      damage=(user.totalhp/6).floor
      if damage>0
        @battle.scene.pbDamageAnimation(user,0)
        user.pbReduceHP(damage)
  ...
  def pbUseMove(choice,specialusage=false)
    ...
    if targets.length==0
      ...
      i=0; loop do break if i>=targets.length
        ...
        # Reset , set Focus Band/Focus Sash to available
        target.damagestate.reset

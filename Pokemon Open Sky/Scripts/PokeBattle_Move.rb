...
################################################################################
  ...
  def pbTypeModifier(type,attacker,opponent)
    ...
    # Dragon Gem
    if opponent.effects[PBEffects::DragonGem]
    ...
    when PBWeather::ANCESTRALAURORA
      if isConst?(atype,PBTypes,:DRAGON)
        mod1=1 if isConst?(otype1,PBTypes,:FAIRY)
        if isConst?(otype2,PBTypes,:FAIRY)
          mod2=(otype1==otype2) ? 2 : 1
        end
        if isConst?(otype3,PBTypes,:DRAGON)
          mod3=(otype1==otype3 || otype2==otype3) ? 2 : 1
        end
      end
    when PBWeather::THUNDERSTORM
      if isConst?(atype,PBTypes,:ELECTRIC)
        mod1=1 if isConst?(otype1,PBTypes,:GROUND)
        if isConst?(otype2,PBTypes,:GROUND)
          mod2=(otype1==otype2) ? 2 : 1
        end
        if isConst?(otype3,PBTypes,:GROUND)
          mod3=(otype1==otype3 || otype2==otype3) ? 2 : 1
        end
      end
    when PBWeather::MAGICWIND
      if isConst?(atype,PBTypes,:FAIRY)
        mod1=1 if isConst?(otype1,PBTypes,:FERAL)
        if isConst?(otype2,PBTypes,:FERAL)
          mod2=(otype1==otype2) ? 2 : 1
        end
        if isConst?(otype3,PBTypes,:FERAL)
          mod3=(otype1==otype3 || otype2==otype3) ? 2 : 1
        end
      end
    end
...
################################################################################
  ...
  def pbAccuracyCheck(attacker,opponent)
    ...
    evastage=0 if opponent.effects[PBEffects::Foresight] ||
                  ...
                  opponent.effects[PBEffects::DragonGem] ||
    ...
    when PBWeather::ANCESTRALAURORA
      if attacker.pbHasType?(:DARK) || attacker.pbHasType?(:FERAL)
        accuracy*=0.9
      elsif attacker.pbHasType?(:DRAGON) || attacker.pbHasType?(:ICE)
        accuracy*=1.5
      end
    when PBWeather::SCARYFOG
      if attacker.pbHasType?(:GHOST) || attacker.pbHasType?(:FLYING)
        accuracy*=1.5
      else
        accuracy*=0.9
      end
    when PBWeather::CROWD
      if attacker.pbHasType?(:PSYCHIC) || attacker.pbHasType?(:DARK)
        accuracy*=0.9
      end
    when PBWeather::STRONGBLIZZARD
      if !attacker.pbHasType?(:ICE) && !attacker.pbHasType?(:FAIRY)
        accuracy*=0.9
      end
    ...
    if attacker.status==PBStatuses::WET
      accuracy*=0.75
    end
    if attacker.status==PBStatuses::BLINDED
      accuracy*=0.75
    end
...
################################################################################
  ...
  def pbCalcDamage(attacker,opponent,options=0)
    ...
    case @battle.pbWeather
    when PBWeather::PLAGUE
      if isConst?(type,PBTypes,:FERAL) || isConst?(type,PBTypes,:FIGHTING)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:BUG) || isConst?(type,PBTypes,:POISON)
        damage=(damage*1.5).round
      end
    when PBWeather::ECLIPSE
      if isConst?(type,PBTypes,:PSYCHIC) || isConst?(type,PBTypes,:FAIRY)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:DARK) || isConst?(type,PBTypes,:GHOST)
        damage=(damage*1.5).round
      end
    when PBWeather::ANCESTRALAURORA
      if isConst?(type,PBTypes,:FAIRY) || isConst?(type,PBTypes,:DARK)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:DRAGON) || isConst?(type,PBTypes,:PSYCHIC)
        damage=(damage*1.5).round
      end
    when PBWeather::THUNDERSTORM
      if isConst?(type,PBTypes,:GROUND) || isConst?(type,PBTypes,:FIRE)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:ELECTRIC) || isConst?(type,PBTypes,:WATER)
        damage=(damage*1.5).round
      end
    when PBWeather::MAGICWIND
      if isConst?(type,PBTypes,:DRAGON) || isConst?(type,PBTypes,:POISON)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:FAIRY) || isConst?(type,PBTypes,:FIGHTING)
        damage=(damage*1.5).round
      end
    when PBWeather::FERALBREEZE
      if isConst?(type,PBTypes,:GHOST) || isConst?(type,PBTypes,:WATER)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:FERAL) || isConst?(type,PBTypes,:GROUND)
        damage=(damage*1.5).round
      end
    when PBWeather::CROWD
      if isConst?(type,PBTypes,:DARK) || isConst?(type,PBTypes,:GHOST)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:FERAL) || isConst?(type,PBTypes,:FIGHTING)
        damage=(damage*1.5).round
      end
    when PBWeather::TORNADO
      if isConst?(type,PBTypes,:ICE) || isConst?(type,PBTypes,:GROUND)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:FLYING) || isConst?(type,PBTypes,:DRAGON)
        damage=(damage*1.5).round
      end
    when PBWeather::SCARYFOG
      if isConst?(type,PBTypes,:FIGHTING) || isConst?(type,PBTypes,:PSYCHIC)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:GHOST) || isConst?(type,PBTypes,:FLYING)
        damage=(damage*1.5).round
      end
    when PBWeather::POLLENSEASON
      if isConst?(type,PBTypes,:ELECTRIC) || isConst?(type,PBTypes,:FERAL)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:GRASS) || isConst?(type,PBTypes,:BUG)
        damage=(damage*1.5).round
      end
    when PBWeather::TREMORS
      if isConst?(type,PBTypes,:POISON) || isConst?(type,PBTypes,:STEEL)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:GROUND) || isConst?(type,PBTypes,:ROCK)
        damage=(damage*1.5).round
      end
    when PBWeather::STRONGBLIZZARD
      if isConst?(type,PBTypes,:GRASS) || isConst?(type,PBTypes,:BUG)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:ICE) || isConst?(type,PBTypes,:FAIRY)
        damage=(damage*1.5).round
      end
    when PBWeather::ACIDRAIN
      if isConst?(type,PBTypes,:ICE) || isConst?(type,PBTypes,:GRASS)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:POISON) || isConst?(type,PBTypes,:FIRE)
        damage=(damage*1.5).round
      end
    when PBWeather::STARRYNIGHT
      if isConst?(type,PBTypes,:ROCK) || isConst?(type,PBTypes,:ELECTRIC)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:PSYCHIC) || isConst?(type,PBTypes,:BUG)
        damage=(damage*1.5).round
      end
    when PBWeather::METEORSHOWER
      if isConst?(type,PBTypes,:BUG) || isConst?(type,PBTypes,:FLYING)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:ROCK) || isConst?(type,PBTypes,:STEEL)
        damage=(damage*1.5).round
      end
    when PBWeather::MAGNETICSTORM
      if isConst?(type,PBTypes,:FLYING) || isConst?(type,PBTypes,:DRAGON)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:STEEL) || isConst?(type,PBTypes,:ELECTRIC)
        damage=(damage*1.5).round
      end
    when PBWeather::SUNNYDAY, PBWeather::HARSHSUN
      if isConst?(type,PBTypes,:FIRE) || isConst?(type,PBTypes,:FERAL)
        damage=(damage*1.5).round
      elsif isConst?(type,PBTypes,:WATER) || isConst?(type,PBTypes,:ICE) 
        damage=(damage*0.5).round
      end
    when PBWeather::RAINDANCE, PBWeather::HEAVYRAIN
      if isConst?(type,PBTypes,:FIRE) || isConst?(type,PBTypes,:GROUND)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:WATER) || isConst?(type,PBTypes,:ICE)
        damage=(damage*1.5).round
      end
    when PBWeather::HAIL
      if isConst?(type,PBTypes,:GRASS) || isConst?(type,PBTypes,:STEEL)
        damage=(damage*0.5).round
      elsif isConst?(type,PBTypes,:ICE) || isConst?(type,PBTypes,:FAIRY)
        damage=(damage*1.5).round
      end
    ...
    # Stung
    if attacker.status==PBStatuses::STUNG && pbIsSpecial?(type) &&
       !attacker.hasWorkingAbility(:GUTS) &&
       !(USENEWBATTLEMECHANICS && @function==0x7E) # Facade
      damage=(damage*0.5).round
    end
    # Petrify
    if opponent.status==PBStatuses::PETRIFY && pbIsPhysical?(type)
      damage=(damage*0.5).round
    end
    # Fear
    if opponent.status==PBStatuses::FEAR && pbIsSpecial?(type)
      damage=(damage*0.5).round
    end
    # Shrapnel
    if opponent.status==PBStatuses::SHRAPNEL && pbIsPhysical?(type)
      damage=(damage*0.33).round
    end
    # Dirty
    if opponent.status==PBStatuses::DIRTY && pbIsSpecial?(type)
      damage=(damage*0.33).round
    end

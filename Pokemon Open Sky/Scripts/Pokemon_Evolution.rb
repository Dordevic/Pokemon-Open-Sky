module PBEvolution
  ...
  EVONAMES=["Unknown",
     ...
     "LevelHoldItem","Event","LevelType","Fainted","Custom5"
  ]
...
def pbMiniCheckEvolution(pokemon,evonib,level,poke)
  ...
  when PBEvolution::Trade, PBEvolution::TradeItem, PBEvolution::TradeSpecies
    return -1
  when PBEvolution::LevelHoldItem
    return poke if pokemon.item==level && pokemon.level>=37
  when PBEvolution::Event
    return -1
  when PBEvolution::LevelType
    if pokemon.level>=40
      for i in $Trainer.pokemonParty
        return poke if i.hasType?(level)
      end
    end
  when PBEvolution::Fainted
    if pokemon.hp==0 && pokemon.level>=level
      pokemon.heal
      return poke
    end
...
def pbMiniCheckEvolutionItem(pokemon,evonib,level,poke,item)
  ...
  when PBEvolution::Event
    return poke if level==item && $game_switches[77]
  end
  return -1
end

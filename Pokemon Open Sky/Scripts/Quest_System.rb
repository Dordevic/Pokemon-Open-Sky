...
def isQuestActive(id)
  return if !$PokemonGlobal
  $PokemonGlobal.quests.isQuestActive(id)
end
...
class QuestScene 
  ...
  def pbDrawNavigationInfo
    ...
    textPositions = [
      ["A,S - Change Section",2,Graphics.height-40,0,baseColor,shadowColor],
  ...
  def pbMain
    loop do
      ...
      if Input.trigger?(Input::LEFT) #X) #A
        @yOffsetInfo=0
        @yOffsetList=0
        for i in 0...@sections.length
        ...
        pbDrawSections
      elsif Input.trigger?(Input::RIGHT)#Y) #S
        @yOffsetInfo=0
        @yOffsetList=0
        for i in 0...@sections.length
          if $PokemonGlobal.selectedSection==@sections[i][0]
